...
proc newPropertyAnimationLayerFrame*(layer: ptr Layer; fromFrame: GRect; toFrame: GRect): PropertyAnimationHandle {.inline.} =
  result = PropertyAnimationHandle(property_animation_create_layer_frame(layer, unsafeAddr fromFrame, unsafeAddr toFrame))

proc newPropertyAnimationBoundsOrigin*(layer: ptr Layer; fromPoint: GPoint; toPoint: GPoint): PropertyAnimationHandle {.inline.} =
  result = PropertyAnimationHandle(property_animation_create_bounds_origin(layer, unsafeAddr fromPoint, unsafeAddr toPoint))
